...
x = exp(1i*2*pi*f*t).*(1-t);    %unit amplitude phasor
xr = real(x);               %Real part
...
plot(t(1:ii), xr(1:ii),'g');             % plot real component against time
hold on;
plot([t(1),t(ii)],[xi(ii),xi(ii)],'r') % plot horizontal line indicating current value of xi
hold off
axis( [ tmin tmax -xmax xmax ]);
axis('square')
xlabel('time');
