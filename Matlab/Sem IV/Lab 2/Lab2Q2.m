...
x = exp(1i*2*pi*f*t);		%unit amplitude phasor
xr = real(x);               %Real part
...
subplot(2,2,3)                          %plot sinusoid on previously set axes
plot(xr(1:ii), t(1:ii),'g');             % plot real component against time
hold on;
plot([xi(ii),xi(ii)], [t(1),t(ii)],'r') % plot horizontal line indicating current value of xi
hold off
axis( [ -xmax xmax tmin tmax ]);
axis('square')
xlabel('real part');
ylabel('time');
title('signal');
...
while te < 0.2
	te = (datetime("now") - t0) * [0 0 86400 3600 60 1]';
end
